...
|made-with-python| |pypi-klan1| |stars| |release| |license|

.. |made-with-python| image:: https://img.shields.io/badge/Made%20with-Python-1f425f.svg
   :target: https://www.python.org/
.. |pypi-klan1| image:: https://img.shields.io/pypi/v/klan1-pproxy
   :target: https://pypi.org/project/klan1-pproxy/
.. |stars| image:: https://img.shields.io/github/stars/klan1/pproxy
   :target: https://github.com/klan1/pproxy
.. |release| image:: https://img.shields.io/github/release/klan1/pproxy
   :target: https://github.com/klan1/pproxy
.. |license| image:: https://img.shields.io/github/license/klan1/pproxy
   :target: https://github.com/klan1/pproxy
...
**``klan1-pproxy``** is a maintained fork of the upstream
`pproxy <https://github.com/qwj/python-proxy>`_ project. The upstream
has been dormant since ``pproxy 2.7.9`` (2024); this fork ships the
fixes needed to run on Python 3.10+ (especially 3.12+ which broke
because of ``asyncio.get_event_loop()``) and is published to PyPI
under the name ``klan1-pproxy``. The import name (``pproxy``) and CLI
(``pproxy``) are unchanged so existing scripts keep working.
...
.. list-table::
   ...
   * - Upstream project
     - https://github.com/qwj/python-proxy (dormant since 2024)
   * - Upstream mirror
     - https://github.com/moreati/pproxy
   * - This fork
     - https://github.com/klan1/pproxy
   * - PyPI package
     - https://pypi.org/project/klan1-pproxy/
   * - Latest release
     - ``klan1-pproxy 3.0.0`` (drops Python 3.6/3.7, works on 3.9–3.13)
   * - Maintainer
     - Alejandro Trujillo J. ``<alejo@klan1.com>``
...
.. code:: rst
  ...
  $ pip3 install klan1-pproxy[accelerated]
...
.. note::
   ...
   The Docker image below (``mosajjal/pproxy``) is **built from the
   upstream ``pproxy 2.x`` series** and therefore contains the old
   ``asyncio.get_event_loop()`` calls that crash on Python 3.12+.
   If you run it on a Python 3.12+ host, the container itself still
   works (it ships its own Python inside), but the upstream image has
   not been rebuilt for several years. For modern Python (3.9+)
   install this fork via PyPI instead::

       pip3 install klan1-pproxy

The legacy ``pproxy`` Docker container has both Python3 (with
Cryptodome for performance optimizations) and ``pypy`` versions
available.
...
Related projects (from upstream ``pproxy`` ecosystem)
------------------------------------------------------

These are **NOT** part of the ``klan1-pproxy`` fork. They are kept here
only because the upstream ``README.rst`` listed them. Both are still
on the legacy ``pproxy 2.x`` line and have not been updated for Python
3.10+.

+ `python-vpn <https://github.com/qwj/python-vpn>`_ - VPN Server (IPSec, IKE, IKEv2, L2TP, WireGuard) in pure Python. By Qian Wenjie.
+ `shadowproxy <https://github.com/guyingbo/shadowproxy>`_ - SOCKS/HTTPS proxy with TLS via gevent. By Gu Yingbo.

Fork maintainer: Alejandro Trujillo J. ``<alejo@klan1.com>``.
